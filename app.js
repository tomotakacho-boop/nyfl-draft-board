...
function renderSnapshots() {
  ...
  const snapshotRows = position => (sub.positions?.[position] || []).slice(0, 12);
  app.innerHTML = `<div class="snapshot-banner"><div><span class="platform">SUBVERTADOWN</span><h2>${sub.status === "connected" ? `Week ${sub.week} snapshot` : "Public preview · import full snapshot"}</h2><p>${escapeHtml(sub.message || "Weekly QB, kicker and defense snapshots are archived after Monday Night Football and again before Sunday kickoff.")}</p><p class="source-note"><a href="https://subvertadown.com/" target="_blank" rel="noreferrer">Open Subvertadown Snapshot ↗</a> · Captured ${escapeHtml(relativeTime(sub.capturedAt))}</p></div><div class="lock-icon">${sub.status === "connected" ? "✓" : "⌁"}</div></div>
    <div class="section-head"><div><h2>${sub.status === "connected" ? "Archived Subvertadown rankings" : "Subvertadown public preview"}</h2><p>${sub.status === "connected" ? "Authorized weekly import · QB, K and D/ST" : "The public page exposes a small preview; the importer archives your subscribed rows without storing your login."}</p></div></div>
    <div class="source-grid">${[["QB","Quarterbacks"],["K","Kickers"],["DEF","D/ST"]].map(([position,label]) => `<article class="source-card"><span class="source-icon">${position}</span><h3>${label}</h3>${snapshotRows(position).map(row => `<p><strong>${escapeHtml(row.player || row.team)}</strong> · ${escapeHtml(row.team || "")} — ${row.projection ? number(row.projection) : "—"} pts <span class="muted-inline">${escapeHtml(row.opponent || "")}</span></p>`).join("") || `<p>No Subvertadown rows imported.</p>`}</article>`).join("")}</div>
    <div class="section-head"><div><h2>Consensus fallback</h2><p>FantasyPros ECR remains available for positions outside the Subvertadown Snapshot.</p></div></div>
    <div class="source-grid">${[["QB","Quarterbacks"],["K","Kickers"],["DEF","D/ST"]].map(([position,label]) => `<article class="source-card"><span class="source-icon">ECR · ${position}</span><h3>${label}</h3>${top(position).map(row => `<p><strong>${escapeHtml(row.positionRank || row.rank)} · ${escapeHtml(row.name)}</strong> — ${row.projection ? number(row.projection) : "—"} pts</p>`).join("") || `<p>Rankings have not posted yet.</p>`}</article>`).join("")}</div>`;
}
